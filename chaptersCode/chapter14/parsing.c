...
/* A pointer to a new empty Qexpr val */
...
void lval_del(lval *v)
{
    ...
    case LVAL_STR:
        free(v->str);
        break;
        /* If Sexpr orr Qexpr then delete all elements inside */
    case LVAL_QEXPR:
    case LVAL_SEXPR:
...
{
    /* Dfferent Types are always unequal */
    if (x->type != y->type)
    {
        return 0;
    }
    ...
    case LVAL_SEXPR:
        ...
        for (int i = 0; i < x->count; i++)
        {
            /* If any elenent not equal then whole list not equal */
...
void lenv_put(lenv *e, lval *k, lval *v)
{
    ...
    for (int i = 0; i < e->count; i++)
    {
        /* If variable is found delete item at that posiion */
...
lval *builtin_load(lenv *e, lval *a)
{
    ...
    if (mpc_parse_contents(a->cell[0]->str, Lispy, &r))
    {
        ...
        /* Delete the expressions and arguments */
        lval_del(expr);
        lval_del(a);
...
lval *lval_call(lenv *e, lval *f, lval *a)
{
    ...
    while (a->count)
    {
        ...
        /* If we ran out f formal arguments to bind */
  ...
  ",
    ...
    if (argc >= 2)
    {
        ...
        for (int i = 1; i < argc; i++)
        {
            ...
            lval *args = lval_add(lval_sexpr(), lval_err(argv[i]));

            /* Pass the builtin load and get the result */
